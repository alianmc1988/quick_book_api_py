...
class Update_Business_Usecase(Base_Use_Case):
    ...
    async def execute(self, business_id:str, business_payload:Update_Business_DTO)-> Business:
        return await self.business_repository.update_business(business_id=business_id,business_payload=business_payload,db=self.db)
